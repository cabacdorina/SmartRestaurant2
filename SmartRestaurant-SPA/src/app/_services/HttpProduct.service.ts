...
export class HttpProductService {
  ...
  updateProduct(prod: Product, oldName: string): Observable<Object> {
    return this.http.put(
      this.baseUrl + "product/UpdateProduct/" + oldName,
      prod
    );
  }
  //------------------------
  ...
  getProductDetails(name: string): Observable<ProductDetails> {
    return this.http.get<ProductDetails>(
      this.baseUrl + "product/GetByName/" + name
    );
  }

  AddSales(prods: Product[]): Observable<Object> {
    return this.http.post(this.baseUrl + "product/AddSales", prods);
  }

  AddProduct(prod: Product): Observable<Object> {
    return this.http.post(this.baseUrl + "product/AddProduct", prod);
  }

  onRemoveProduct(prodName: string): Observable<Object> {
    return this.http.delete(
      this.baseUrl + "product/DeteleProdByName/" + prodName
    );
  }
